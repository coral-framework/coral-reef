function bulkReferencing()
	...
	setup:publishForAll( "stubs.TestComponent", "key" )
	
	-- Cascade calling test
	local ref = {}
	local client = setup:getNode( 1 )
	for i = 2, 10 do
		ref[i] = client:findRemoteInstance( "stubs.TestComponent", "key"..i, "address"..i ).reference
	end
	...
	-- Ping Pong test
	local tcIn2 = client:newRemoteInstance( "stubs.TestComponent", "address2" )
